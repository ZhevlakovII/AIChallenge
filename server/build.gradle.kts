...
// ===== Анализ зависимостей классов (jdeps) для модуля server =====

val moduleName = "server"
...
tasks.register<Exec>("jdepsClassGraph") {
    ...
    val jarFile = tasks.named("jar").flatMap { (it as org.gradle.api.tasks.bundling.Jar).archiveFile }
    val runtimeCp = configurations.getByName("runtimeClasspath").asPath
